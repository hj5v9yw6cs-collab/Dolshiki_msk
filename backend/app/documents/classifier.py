...
# ДДУ. У претензии и иска над названием стоит шапка с адресатом и сторонами,
# поэтому строк смотрим много — но заголовком считается только короткая
# строка: в теле документа те же слова идут в длинных предложениях.
TITLE_LINES = 40
...
def headings(text: str) -> List[str]:
    """Строки-кандидаты в заголовок документа.

    Разбор идёт по исходному тексту, а не по схлопнутому: переводы строк
    здесь и есть разметка. Название редко стоит первой строкой — выше него
    бывают город с датой, адресат, реквизиты сторон, — поэтому смотрим
    начало документа целиком, но берём только короткие строки. Склейка
    первых трёх нужна отдельно: распознанный PDF рвёт длинное название.
    """
    lines = [normalize(line).strip() for line in (text or "").splitlines()]
    lines = [line for line in lines if line][:TITLE_LINES]
    # Строка с тире посередине — определение из раздела терминов
    # («Акт приема-передачи – документ, которым…»), а не заголовок.
    # Без этой проверки договор с разделом терминов читался как акт.
    short = [
        line for line in lines
        if len(line) <= TITLE_MAX_LENGTH and not DEFINITION_RE.search(line)
    ]
    return short + [" ".join(lines[:3])]
...
def _score(
    ...
    # Стоп-слово отменяет тип, когда оно стоит в заголовке: «дополнительное
    # соглашение» — это не ДДУ. По всему тексту проверять нельзя: договор
    # упоминает и уступку, и акт приёма-передачи, и любое из этих слов
    # обнуляло правильный тип целиком.
    for stop in doc_type.stop_words:
        needle = normalize(stop)
        if needle in filename or any(line.startswith(needle) for line in title_lines):
            return 0.0, []
